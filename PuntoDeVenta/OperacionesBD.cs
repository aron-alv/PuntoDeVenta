...
namespace PuntoDeVenta
{
    public class OperacionesBD
    {
        ...
        public bool AgregarDetalleInventario(int ID_Inventario, int ID_Producto, int Cantidad_Entrante, decimal Costo_Unitario, decimal Subtotal)
        {
            try
            {
                ...
                using (SqlTransaction transaction = connection.BeginTransaction())
                {
                    try
                    {
                        //  agregar el detalle del inventario
                        using (SqlCommand cmd = new SqlCommand("INSERT INTO DetalleInventario (ID_Inventario, ID_Producto, Cantidad_Entrante, Costo_Unitario, Subtotal) VALUES (@ID_Inventario, @ID_Producto, @Cantidad_Entrante, @Costo_Unitario, @Subtotal)", connection, transaction))
                        {
                            cmd.Parameters.AddWithValue("@ID_Inventario", ID_Inventario);
                            cmd.Parameters.AddWithValue("@ID_Producto", ID_Producto);
                            cmd.Parameters.AddWithValue("@Cantidad_Entrante", Cantidad_Entrante);
                        ...
                        using (SqlCommand commandExistencia = new SqlCommand(queryExistencia, connection, transaction))
                        {
                            ...
                            {
                                // Si no existe un nuevo registro en Saldos 
                                string queryInsertar = "INSERT INTO Saldos (ID_Producto, Cantidad_Entrante, Cantidad_Salida) " +
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show("Error al agregar detalle de inventario: " + ex.Message);
                return false;
            }
            finally
            {
